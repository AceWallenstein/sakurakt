...
import com.blankspace.sakura.databinding.FragmentWebBookBinding
import com.just.agentweb.AgentWeb
import com.just.agentweb.DefaultWebClient
import com.just.agentweb.WebChromeClient
import com.just.agentweb.WebViewClient

class WebBookFragment : BaseFragment<FragmentWebBookBinding>() {
    private lateinit var agentWeb:AgentWeb
    private val url = "https://m.readnovel.com/"

    override fun getViewBinding(
        inflater: LayoutInflater,
        container: ViewGroup?
    ): FragmentWebBookBinding {
        return FragmentWebBookBinding.inflate(inflater)
    }

    override fun initView(vb: FragmentWebBookBinding) {
        with(vb){
            {

                ivHome.setOnClickListener {
                    toast(requireContext(),"home")
                    if (agentWeb != null) {
                        if (!agentWeb!!.back()) {
//                            finish()
                        }}
...
//        }
        agentWeb = AgentWeb.with(this)
            ...
            .useDefaultIndicator(requireActivity().getColor(R.color.textColorPrimary), 2)
            .interceptUnkownUrl()
